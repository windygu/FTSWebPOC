...
namespace FTS.BaseBusiness.Systems {
    ...
    public class ReportPeriod {
        ...
        public ReportPeriod(string reportPeriodName, DateTime daystartoffirstyear, DateTime dayStart, DateTime dayEnd,
            ...
            if (ftsMain.Language == Language.VN) {
                ...
                for (int i = 1; i <= 12; i++) {
                    ...
                    if (ftsMain.DayStartOfCurrentYear.Day <= 15) {
                        kybaocao =
                            new ReportPeriod(
                                lblthang + " " + dauthang.Month.ToString() + " " + lblnam + " " +
                                dauthang.Year.ToString(), ftsMain.DayStartOfFirstYear, dauthang, cuoithang,
                                ftsMain.DayStartOfCurrentYear);
                    } else {
                        kybaocao =
                            new ReportPeriod(
                                lblthang + " " + cuoithang.Month.ToString() + " " + lblnam + " " +
                                cuoithang.Year.ToString(), ftsMain.DayStartOfFirstYear, dauthang, cuoithang,
                                ftsMain.DayStartOfCurrentYear);
                    }
                    array.Add(kybaocao);
                }
                for (int i = 1; i <= 4; i++) {
                    ...
                    if (ftsMain.DayStartOfCurrentYear.Day <= 15) {
                        kybaocao =
                            new ReportPeriod(quy + " " + i.ToString() + " " + lblnam + " " + dauquy.Year.ToString(),
                                             ftsMain.DayStartOfFirstYear, dauquy, cuoiquy, ftsMain.DayStartOfCurrentYear);
                    } else {
                        kybaocao =
                            new ReportPeriod(quy + " " + i.ToString() + " " + lblnam + " " + cuoiquy.Year.ToString(),
                                             ftsMain.DayStartOfFirstYear, dauquy, cuoiquy, ftsMain.DayStartOfCurrentYear);
                    }
                    array.Add(kybaocao);
                }
                kybaocao =
                    new ReportPeriod(
                        "6 " + lblthang + " " + ftsMain.DayStartOfCurrentYear.AddYears(1).AddDays(-1).Year,
                        ftsMain.DayStartOfCurrentYear, ftsMain.DayStartOfCurrentYear,
                        ftsMain.DayStartOfCurrentYear.AddMonths(6).AddDays(-1), ftsMain.DayStartOfCurrentYear);
                array.Add(kybaocao);
                kybaocao =
                    new ReportPeriod(
                        "9 " + lblthang + " " + ftsMain.DayStartOfCurrentYear.AddYears(1).AddDays(-1).Year,
                        ftsMain.DayStartOfCurrentYear, ftsMain.DayStartOfCurrentYear,
                        ftsMain.DayStartOfCurrentYear.AddMonths(9).AddDays(-1), ftsMain.DayStartOfCurrentYear);
                array.Add(kybaocao);
                kybaocao = new ReportPeriod(ca_nam + " " + ftsMain.DayStartOfCurrentYear.AddYears(1).AddDays(-1).Year,
                                            ftsMain.DayStartOfFirstYear, ftsMain.DayStartOfCurrentYear,
                                            ftsMain.DayStartOfCurrentYear.AddYears(1).AddDays(-1),
                                            ftsMain.DayStartOfCurrentYear);
                array.Add(kybaocao);
            } else {
                for (int i = 1; i <= 12; i++) {
                    ...
                    DateTime cuoithang = dauthang.AddMonths(1).AddDays(-1);
                    if (ftsMain.DayStartOfCurrentYear.Day <= 15) {
                        kybaocao = new ReportPeriod(GetMonth(ftsMain, dauthang.Month) + " " + dauthang.Year.ToString(),
                                                    ftsMain.DayStartOfFirstYear, dauthang, cuoithang,
                                                    ftsMain.DayStartOfCurrentYear);
                    } else {
                        kybaocao =
                            new ReportPeriod(
                                GetMonth(ftsMain, cuoithang.Month) + " " + " " + cuoithang.Year.ToString(),
                ...
                for (int i = 1; i <= 4; i++) {
                    ...
                    if (ftsMain.DayStartOfCurrentYear.Year <= 15) {
                        kybaocao = new ReportPeriod(GetQuarter(ftsMain, i - 1) + " " + dauquy.Year.ToString(),
                                                    ftsMain.DayStartOfFirstYear, dauquy, cuoiquy,
                                                    ftsMain.DayStartOfCurrentYear);
                    } else {
                        kybaocao = new ReportPeriod(GetQuarter(ftsMain, i - 1) + " " + cuoiquy.Year.ToString(),
                                                    ftsMain.DayStartOfFirstYear, dauquy, cuoiquy,
                                                    ftsMain.DayStartOfCurrentYear);
                    }
                    array.Add(kybaocao);
                ...
                kybaocao =
                    new ReportPeriod(
                        ftsMain.MsgManager.GetMessage("MSG_ALL_YEAR") + " " +
                        ftsMain.DayStartOfCurrentYear.AddYears(1).AddDays(-1).Year, ftsMain.DayStartOfFirstYear,
                        ftsMain.DayStartOfCurrentYear, ftsMain.DayEndOfCurrentYear, ftsMain.DayStartOfCurrentYear);
                array.Add(kybaocao);
            }
            return array;
        }
            ...
            if (ftsMain.Language == Language.VN) {
                ...
                kybaocao =
                    new ReportPeriod(
                        ftsMain.MsgManager.GetMessage("MSG_ALL_YEAR") + " " +
                        ftsMain.DayStartOfCurrentYear.AddYears(1).AddDays(-1).Year, ftsMain.DayStartOfFirstYear,
                        ftsMain.DayStartOfCurrentYear, ftsMain.DayStartOfCurrentYear.AddYears(1).AddDays(-1),
                        ftsMain.DayStartOfCurrentYear);
                array.Add(kybaocao);
            }
            return array;
        ...
        public DateTime DayStartOfQuarter {
            get {
                int sothang = this.mDayStart.Month - this.DayStartOfCurrentYear.Month +
                              12*(this.mDayStart.Year - this.DayStartOfCurrentYear.Year);
                if (sothang < 0) {
